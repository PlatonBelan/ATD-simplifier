...
def test_adapter_good(adapter):
    adapter.simplify("./test/test_data.shp", 10, "./data/result_10%.shp")


def test_input_path_wrong(adapter):
    try:
        adapter.simplify("wrong_file.shp", 10, "./data/result_10%.shp")
...
def test_percentage_good(adapter, percentage):
    adapter.simplify("./test/test_data.shp", percentage,
                     "./data/result_10%.shp")
...
def test_output_format_wrong(adapter):
    try:
        adapter.simplify("./test/test_data.shp", 10, "./data/result_10%.shape")
    except ValueError:
